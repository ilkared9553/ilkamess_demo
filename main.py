...
def clean_secure_path(path_str):
    ...
    if not path_str:
        return ""
    cleaned = re.sub(r'[\x00-\x1f\x7f-\x9f]', '', path_str)
...
class MainChatsScreen(MDScreen):
    ...
    def load_chats(self):
        ...
        for item in all_items:
            ...
            if item["type"] == 'channel':
                last_msg = f"Канал ({item['sub_count']} подп.): {last_msg}"
        ...
        if search_term.startswith('@'):
            ...
            if user_found:
                ...
                item = TwoLineAvatarIconListItem(
                    ...
                    secondary_text=f"Пользователь: @{user_found['username']}",
        ...
        # ЕСЛИ БЕЗ @ -> ВЫДАЕМ ПОДСКАЗКУ И ИЩЕМ ТОЛЬКО КАНАЛЫ
            ...
            for channel_found in channels_found:
                ...
                item = TwoLineAvatarIconListItem(
                    ...
                    secondary_text=f"Публичный канал: @{channel_found['username']}",
    ...
    ### РЕАЛИЗАЦИЯ МЕНЮ ВЫБОРА ЧАТ/КАНАЛ НА КНОПКЕ ПЛЮС ###
    ...
    def show_create_chat_dialog(self):
        self.chat_user_field = Builder.load_string('MDTextField:\n  hint_text: "Юзернейм собеседника (без @)"')
    ...
    def show_creation_dialog(self):
        self.chan_title_field = Builder.load_string('MDTextField:\n  hint_text: "Название канала"')
        self.chan_user_field = Builder.load_string('MDTextField:\n  hint_text: "Юзернейм канала (без @)"')
...
class ChatScreen(MDScreen):
    ...
    def show_rename_dialog(self):
        if self.chat_type != 'private':
            ...
            return
        self.rename_field = Builder.load_string('MDTextField:\n  hint_text: "Локальный псевдоним"')
...
class SettingsScreen(MDScreen):
    ...
    def load_user_data(self):
        ...
        if not app.current_user:
            return
        self.ids.current_username.text = f"Текущий ID: @{app.current_user['username']}"
